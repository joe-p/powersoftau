...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() != 4 {
        eprintln!(
            "Usage: {} <input_challenge> <output_challenge> <target_power>",
            args[0]
        );
    ...
    let target_power: usize = args[3]
        .parse()
        .expect("target_power must be a valid number");
    ...
    println!(
        "Reducing challenge from input file '{}' to target power 2^{} in output file '{}'",
    ...
    println!(
        "  Current power: 2^{} (tau_powers_length: {})",
        current_power, current_tau_powers_length
    );
    ...
    if target_power > current_power {
        eprintln!(
            "Error: target power 2^{} is larger than input power 2^{}",
    ...
    println!(
        "  tau_powers_g1: {} -> {}",
        current_tau_powers_g1_length, target_tau_powers_g1_length
    );
    println!(
        "  tau_powers_g2: {} -> {}",
        current_tau_powers_length, target_tau_powers_length
    );
    println!(
        "  alpha_tau_powers_g1: {} -> {}",
        current_tau_powers_length, target_tau_powers_length
    ...
    input
        .read_exact(&mut hash)
        .expect("unable to read hash from input file");
    output
        .write_all(&hash)
        .expect("unable to write hash to output file");

    // Stream copy tau_powers_g1 (first target_tau_powers_g1_length points)
    println!("Copying tau_powers_g1...");
    stream_copy_points::<G1Affine, _>(
        &mut input,
        &mut output,
        target_tau_powers_g1_length,
        Bls12CeremonyParameters::G1_UNCOMPRESSED_BYTE_SIZE,
    )?;
    ...
    stream_copy_points::<G2Affine, _>(
        &mut input,
        &mut output,
        target_tau_powers_length,
...
fn detect_power_from_size(accumulator_size: usize, _parameters: &Bls12CeremonyParameters) -> usize {
    ...
    for power in 10..=27 {
        ...
        let expected_size = (tau_powers_g1_length
            * Bls12CeremonyParameters::G1_UNCOMPRESSED_BYTE_SIZE)
    ...
    panic!(
        "Could not detect power from file size {} bytes",
        accumulator_size
    );
}
...
fn verify_reduced_challenge(
    ...
    println!(
        "Reading and verifying tau_powers_g1 ({} points)...",
        expected_tau_powers_g1_length
    );
    verify_g1_points(&mut reader, expected_tau_powers_g1_length)?;

    println!(
        "Reading and verifying tau_powers_g2 ({} points)...",
        expected_tau_powers_length
    );
    verify_g2_points(&mut reader, expected_tau_powers_length)?;

    println!(
        "Reading and verifying alpha_tau_powers_g1 ({} points)...",
        expected_tau_powers_length
    );
    verify_g1_points(&mut reader, expected_tau_powers_length)?;

    println!(
        "Reading and verifying beta_tau_powers_g1 ({} points)...",
        expected_tau_powers_length
    );
    verify_g1_points(&mut reader, expected_tau_powers_length)?;
    ...
    println!(
        "  - Accumulator structure is correct for 2^{}",
        target_power
    );
